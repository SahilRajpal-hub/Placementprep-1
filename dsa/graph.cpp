...
// 1. Number of Islands
bool isValidIndex(int i, int j, int n, int m,vector<vector<char>>& grid, vector<vector<bool>>& visited){
    if(i<n && j<m && i>=0 && j>=0 && grid[i][j]=='1' && !visited[i][j]) return true;
    return false;
}
void dfs(vector<vector<char>>& grid, vector<vector<bool>>& visited, int i, int j){
    ...
    if(isValidIndex(i-1,j,n,m,grid,visited)) dfs(grid,visited,i-1,j);
    if(isValidIndex(i+1,j,n,m,grid,visited)) dfs(grid,visited,i+1,j);
    if(isValidIndex(i,j-1,n,m,grid,visited)) dfs(grid,visited,i,j-1);
    if(isValidIndex(i,j+1,n,m,grid,visited)) dfs(grid,visited,i,j+1);
}
...
//2. Flood Fill
bool isValidIndex(int i, int j, int n, int m,vector<vector<int>>& image, int ec){
    if(i<n && j<m && i>=0 && j>=0 && image[i][j]==ec) return true;
    return false;
}
void dfs(vector<vector<int>>& image, int i, int j, int ec, int nc){
    image[i][j]=nc;
    int n = image.size();
    int m = image[0].size();
    if(isValidIndex(i-1,j,n,m,image,ec)) dfs(image,i-1,j,ec,nc);
    if(isValidIndex(i+1,j,n,m,image,ec)) dfs(image,i+1,j,ec,nc);
    if(isValidIndex(i,j-1,n,m,image,ec)) dfs(image,i,j-1,ec,nc);
    if(isValidIndex(i,j+1,n,m,image,ec)) dfs(image,i,j+1,ec,nc);
}
...
//3. Rotten Oranges (Only possible with Breadth first search)
bool isValidIndex(int i, int j, int n, int m,vector<vector<int>>& grid,vector<vector<bool>>& visited){
    if(i<n && j<m && i>=0 && j>=0 && grid[i][j]==1 && !visited[i][j]) return true;
    return false;
}
void bfs(vector<vector<int>>& grid,vector<vector<bool>>& visited, int &ans){
    queue<vector<int>> q;
    int n = grid.size();
    int m = grid[0].size();
    ...
    while(!q.empty()){
        ...
        if(isValidIndex(tmp[0]+1,tmp[1],n,m,grid,visited)){
        ...
        if(isValidIndex(tmp[0],tmp[1]+1,n,m,grid,visited)){
...
// ---> DFS
bool dfs(vector<int> adj[], vector<bool>& visited , vector<bool>& pathVisited, int sn){
    visited[sn]=true;
    pathVisited[sn]=true;
    
    for(int ed:adj[sn]){
        if(!visited[ed]){
            bool ans = dfs(adj,visited,pathVisited,ed);
            if(ans) return true;
        }else if(pathVisited[ed]){
            return true;
        }
    }
    
    pathVisited[sn]=false;
    return false;
}
...
A linear graph with no cycle is always bipartite.

Any graph with even number of nodes is bipartite.

Any graph with odd number of nodes is *never* bipartite.
...
// *** topological sort using BFS using Kahn's algo / also detect cycle in directed graph using BFS
vector<int> topoSort(int V, vector<int> adj[])
{
    vector<int> indegree(V, 0);
    vector<int> ans;
